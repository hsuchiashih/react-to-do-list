...
function Todo() {
  ...
  const [unfinishedCount, setFinishedCountState] = useState(0);
  ...
  const getTodos = async () => {
    ...
    try {
      ...
      const unfinishedCount = response.data.data.filter(item => item.status === false).length
      const todoItemCount = response.data.data.length === 0
      setTodoList(response.data.data)
      setFinishedCountState(unfinishedCount)
